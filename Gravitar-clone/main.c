...
int main() {
    ...
    size_t highscore = 0;
    bool nuevo_highscore = false;

    while (jugar_de_nuevo) {
        nuevo_highscore = false;
        ...
        for (estadio_t i = INICIO; i <= NIVEL5; i++) {
            if (niveles[i] == NULL) return 1;
        }
        ...
        */
        ...
        while (1) {
            ...
            if (nivel_nav == INICIO) {
                ...
                if (nivel_estrella_atrae_nave(nivel_act, jugador)) {
            ...
            //Chequea finalizacion del nivel
            if (!niv_derrotado && nivel_vencido(nivel_act, jugador)) {
                niv_derrotado = true;
            }

            //Chequea nuevo highscore
            if (highscore < puntos_nav) {
                nuevo_highscore = true;
                highscore = puntos_nav;
            }
            ...
            if (vidas_nav <= 0) {
                ...
                game_over = true;
                if (nuevo_highscore)
                    dibujar_texto("NEW HIGH SCORE", VENTANA_ANCHO / 2 - 165, VENTANA_ALTO / 2 - 85, 4, 1, 0, 1, VENTANA_ALTO, renderer);
                nave_setear_pos(jugador, base_get_posx(nivel_get_base(niveles[INICIO])), base_get_posy(nivel_get_base(niveles[INICIO])));
